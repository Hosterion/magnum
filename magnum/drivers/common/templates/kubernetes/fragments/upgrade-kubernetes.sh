...
if [ "${new_kube_tag}" != "${KUBE_TAG}" ]; then
    ...
    if [ "$(echo $USE_PODMAN | tr '[:upper:]' '[:lower:]')" == "true" ]; then
        ...
        ${ssh_cmd} podman rmi ${CONTAINER_INFRA_PREFIX:-ghcr.io/hosterion/}hyperkube:${KUBE_TAG}
        ...
        i=0
        until [ "`${ssh_cmd} podman image exists ${CONTAINER_INFRA_PREFIX:-ghcr.io/hosterion/}hyperkube:${new_kube_tag} && echo $?`" = 0 ]
        do
            i=$((i+1))
            [ $i -lt 30 ] || break;
        ...
        KUBE_DIGEST=$($ssh_cmd podman image inspect ${CONTAINER_INFRA_PREFIX:-ghcr.io/hosterion/}hyperkube:${new_kube_tag} --format "{{.Digest}}")
